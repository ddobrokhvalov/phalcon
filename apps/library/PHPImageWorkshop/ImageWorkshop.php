...
namespace Multiple\Library\PHPImageWorkshop;
...
class ImageWorkshop
{
    ...
    public static function initFromPath($path, $fixOrientation = false)
    {
        ...
        if (false === ($imageSizeInfos = @getImageSize($path))) {
           ...
           return array(
               'error' => true,
               'msg'   => 'Невозможно открыть файл. Вероятно файл не является изображением.'
           );
        }
        ...
        if (!$mimeContentType || !isset($mimeContentType[1])) {
           // throw new ImageWorkshopException('Not an image file (jpeg/png/gif) at "'.$path.'"', static::ERROR_NOT_AN_IMAGE_FILE);
            return array(
                'error' => true,
                'msg'   => 'Неверный формат. Изображение должно быть формата jpeg/jpg/png/gif'
            );
        }
        ...
        if (false === $image) {
           // throw new ImageWorkshopException('Unable to create image with file found at "'.$path.'"');
            return array(
                'error' => true,
                'msg'   => 'Неизвестная ошибка. Обратитесь к администратору. Unable to create image with file found at '.$path
            );
        }
