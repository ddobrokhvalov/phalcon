...
Для проверки выбрать раздел "отсоединенная, в формате <b>«PKCS#7»</b>, загрузить 2 файла (файл жалобы «docx» и файл подписи «docx.sig»), после чего нажать «проверить».<br/>'.$add_content;
    ...
    public function recallChangeStaAndSendUfasAction()
    {
        ...
        $ufas = Ufas::findFirst($complaint->ufas_id);
         ...
         $content = 'Просим отозвать поданную ранее жалобу на закупку. Прилагаемый файл отзыва жалобы в формате «docx» подписан квалифицированной <b>отсоединенной подписью</b>, которая создается отдельно от подписываемого файла.<br/><br/>
Файл подписи в формате «docx.sig».<br/><br/>
Действительность подписи можно проверить на сайте Правительства РФ «ГОСУСЛУГИ» - https://www.gosuslugi.ru/pgu/eds/.<br/> 
Для проверки выбрать раздел "отсоединенная, в формате <b>«PKCS#7»</b>, загрузить 2 файла (файл отзыва жалобы «docx» и файл подписи «docx.sig»), после чего нажать «проверить».<br/>';
