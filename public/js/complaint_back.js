...
var complaint = {
    ...
    checkApplicant: function(){
		if (!$("#user_id").val()) {
            ...
            showStyledPopupMessage("#pop-before-ask-question", "Ошибка", "Пользователь не выбран");
            return false;
        }
        if (!$("#applicant_id").val()) {
            //$("html, body").animate({ scrollTop: 0 }, 1);
                ...
                if (!window.edit_mode) {
                    ...
                    $.ajax({
                        ...
                        success: function (res) {
                            ...
                            createDocx(compID)
                            setTimeout(function () {
                                location.href = '/admin/complaints/preview/' + compID;
                            }, 1000);
                        }
                    });
                } else {
...
function saveComplaintToDocxFile() {
    ...
    $.ajax({
        url:  "/admin/complaints/saveHtmlFile",
        type: 'POST',
        data: data,
        async: false,
        cache: false,
